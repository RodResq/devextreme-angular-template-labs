...
export abstract class CrudService<T> {
  ...
  public findAll(params: string = ""): Observable<Page<T>> {
    return this._http.get<Page<T>>(`${environment.urlBase}${this.apiContext}`);
  }
  ...
  public filter(sort: string, page: number = 0, size: number = 20, jiiPayload?: JiiPayload): Observable<JiiPage<T>> {
    return this._http.post<JiiPage<T>>(`${environment.urlBase}${this.apiContext}?sort=${sort}&page=${page}&size=${size}`, jiiPayload);
  }
