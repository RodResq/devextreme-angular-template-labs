...
export class JiiStore<T> extends CustomStore {
   ...
   * @param basePath 
   */
  private loadUrl(options: JiiStoreOptions, basePath: string): string {
    if (options.loadByGet) return `${environment.urlBase}${options.endpoints.load || basePath}`;
    return `${environment.urlBase}${options.endpoints.load || `${basePath}`}`;
  }
